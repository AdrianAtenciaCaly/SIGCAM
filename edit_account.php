...
if (isset($_POST['update'])) {
  ...
  if (empty($errors)) {
    ...
    $name = removejunk($db->escape($_POST['name']));
    $username = removejunk($db->escape($_POST['username']));
    $sql = "UPDATE users SET name ='{$name}', iduser ='{$username}' WHERE id='{$id}'";
    $result = $db->query($sql);
      ...
      <div class="panel-body">
        <form method="post" action="edit_account.php?id=<?php echo (int) $user['id']; ?>" class="clearfix">
          <div class="form-group">
            ...
            <input type="name" class="form-control" name="name" value="<?php echo removejunk(ucwords($user['name'])); ?>">
          </div>
          <div class="form-group">
            <label for="username" class="control-label">Usuario</label>
            <input type="text" class="form-control" name="username" value="<?php echo removejunk(ucwords($user['iduser'])); ?>">
          </div>
